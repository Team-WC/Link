...
{
    public GameObject PostsButton;
    public GameObject UsersButton;
    public GameObject AlarmsButton;
    ...
    void Start()
    {
        ...
        if (PostsButton)
            PostsButton.GetComponent<Button>().onClick.AddListener(() => { GoToScreen(0); });
        if (UsersButton)
            UsersButton.GetComponent<Button>().onClick.AddListener(() => { GoToScreen(1); });
        if (AlarmsButton)
            AlarmsButton.GetComponent<Button>().onClick.AddListener(() => { GoToScreen(2); });
        if (MessagesButton)
            MessagesButton.GetComponent<Button>().onClick.AddListener(() => { GoToScreen(3); });
    }
